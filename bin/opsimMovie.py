...
def setupMovieSlicer(simdata, bins, verbose=False):
    ...
    movieslicer = slicers.MovieSlicer(
        sliceColName='expMJD', bins=bins, cumulative=True)
...
def runSlices(opsimName, metadata, simdata, fields, bins, args, opsDb, verbose=False, onlyFilterColors=True):
    ...
    for i, ms in enumerate(movieslicer):
        ...
        times_from_start = ms['slicePoint'][
            'binRight'] - (int(bins[0]) + 0.16 - 0.5)
        ...
        for metric, plotDict in zip(metricList, plotDictList):
            bundles.append(metricBundles.MetricBundle(metric, opslicer, sqlconstraint=sqlconstraint,
                                                      ...
                                                      plotDict=plotDict))
        # Remove (default) stackers from bundles, because we've already run
        # them above on the original data.
        for mb in bundles:
            mb.stackerList = []
        ...
        bg.runCurrent(sqlconstraint, simData=simData)
        ...
        ph = plots.PlotHandler(
            outDir=args.outDir, figformat='png', dpi=72, thumbnail=False, savefig=False)
        obsnow = np.where(simdatasubset['expMJD']
                          == simdatasubset['expMJD'].max())[0]
        raCen = np.mean(simdatasubset[obsnow]['lst'])
        # Calculate horizon location.
        horizonlon, horizonlat = addHorizon(
            lat_telescope=lat_tele, raCen=raCen)
        # Create the plot for each metric and save it (after some additional
        # manipulation).
        for mb in bundles:
            ...
            if mb.metric.name == 'FilterColors':
                ...
                circle = Circle((lon, moonDec), radius=0.05,
                                color='k', alpha=alpha)
                ax.add_patch(circle)
                ...
                zenith = Line2D([], [], color='k', linestyle='',
                                marker='+', markersize=5, label="Zenith")
            ...
            # Save figure.
            plt.savefig(os.path.join(args.outDir, mb.metric.name +
                                     '_' + slicenumber + '_SkyMap.png'), format='png', dpi=72)
            plt.close('all')
            dt, t = dtime(t)
        if verbose:
...
def stitchMovie(metricList, args):
    ...
    for metric in metricList:
        ...
        plotfiles = fnmatch.filter(os.listdir(
            args.outDir), outfileroot + '*_SkyMap.png')
        slicenum = plotfiles[0].replace(outfileroot, '').replace(
            '_SkyMap.png', '').replace('_', '')
...
if __name__ == '__main__':
    ...
    opsimName = args.opsimDb.replace('.db', '')
    opsimName = opsimName.replace('_sqlite', '')
    ...
    if not args.skipComp:
        ...
        oo = db.OpsimDatabase(dbfile,
                              defaultdbTables={'Summary': ['Summary', 'obsHistID'], 'Field': ['Field', 'fieldID']})
        oo.raColName = 'fieldRA'
        oo.decColName = 'fieldDec'
